...
**UI**: Chainlit  
**Architecture**: Observer / Pub-Sub with Blackboard + SKILLS.md-Driven Agent Registry  
...
### 4.2 MAS Design Patterns

This system deliberately combines four distinct multi-agent design patterns. Understanding
which pattern governs which part of the pipeline is essential — they apply to different
concerns and operate at different levels.

---

#### Pattern 1 — Observer / Pub-Sub (Primary Dispatch Mechanism)

The research cascade implements the **Observer pattern**. Its mechanism is a `MessageHub`
— a dictionary that maps message types to lists of async handler functions.
...
The **Blackboard** pattern provides a shared, typed data space that all agents write to
and read from, without any agent needing to know who else is accessing it.
...
- Agents are decoupled from each other — no agent holds a reference to another
- Any agent that needs data from a completed agent reads it directly off the board
- The board is the single source of truth for all LLM-produced research outputs
- `None` fields are a first-class state: they signal missing or failed sections to
  downstream agents like `ScoringAgent`
- The board persists in the Chainlit session after pipeline completion, enabling
  the conversational overlay (Section 4.6)
...
#### Pattern 3 — Quorum Gate / Counter-Based Aggregator (ScoringAgent)

This pattern is not present in the reference system and is introduced here specifically
for the partial-failure tolerance requirement.
...
**What this pattern provides:**
- The pipeline always completes — no agent failure causes a deadlock
- Partial results produce a partial report with clear confidence flagging
- The gate threshold (`expected_sections = 7`) is the only coordination primitive
  needed — no complex orchestration, no timeouts, no retry logic at the gate level
- Pass/fail ratio is irrelevant to gate opening; only count matters

---

#### Pattern 4 — SKILLS.md-Driven Agent Registry (Self-Configuring Agents)

**No research agents are hardcoded in Python.** Agent behaviour, identity, and tool
assignments come from `SKILLS.md` files — one per agent, living in `skills/<agent_key>/`.
The registry scans this directory at startup and constructs agents automatically.

This pattern is adopted from the reference Customer Service MAS architecture
(SPECIFICATIONS_3a) and adapted here for the research domain.

**Two agent types exist in this system:**

**Research agents** (all section agents: career, background, rankings, program,
employability, accommodation, news, forum):
- Their tools are external MCP servers (Tavily, Fetch, Brave, SerpAPI)
- They need: the shared MCP server reference + behavioural instructions
- Both come from their `SKILLS.md` file
- The registry builds them automatically as `SpecialistAgent` instances
- **To add a new research agent: drop a `SKILLS.md` in `skills/`, restart. No Python.**

**Hardcoded agents** (scoring, alternatives, report generator, conversation):
- Their logic is non-trivial Python — counter state, Jinja2 rendering, session management
- A `SKILLS.md` file provides their behavioural instructions; the Python class provides
  their logic
- They are registered explicitly in `build_registry()`, then supplemented by their
  `SKILLS.md` if one exists
- **To add a new hardcoded agent: write the Python class AND a `SKILLS.md`**

**The `SKILLS.md` file format:**

Each file has two parts separated by `---` delimiters:
...
type: research
description: Researches university history, orientation, and known strengths for the course.
---

## Behaviour

- Always scope searches to the specific course, not the university generally.
- Prefer official university sources over third-party aggregators for factual claims.
- Flag any accreditations that are course-specific vs university-wide.

## Edge Cases

- University not found in search results — return confidence: low with explanation.
- Generic university profile only (no course-specific detail) — flag in output.
```

The YAML frontmatter is machine-readable metadata. The markdown body after the second
`---` is the agent's behavioural instructions, injected into its system prompt at startup.

**The `SkillMeta` dataclass** (parsed from each file):

```python
@dataclass
class SkillMeta:
    key:          str        # "background" — matches skills/ subdirectory name
    name:         str        # "Background Agent"
    type:         str        # "research" or "hardcoded"
    description:  str        # used by supervisor routing prompt
    instructions: str        # full markdown body — injected into system prompt
```

**The registry build sequence at startup:**

```
1. Hardcoded agents registered first (scoring, alternatives, report_generator,
   conversation) with their Python classes. Their SKILLS.md loaded for instructions.

2. skills/ directory scanned. Every type:research entry becomes a SpecialistAgent.
   Every type:hardcoded entry not already registered is skipped with a warning.

3. Registry build complete — all agents available for subscription wiring.
```

**Startup log sequence:**

```
skill_loader | loaded skills/career/SKILLS.md
skill_loader | loaded skills/background/SKILLS.md
skill_loader | loaded skills/rankings/SKILLS.md
skill_loader | loaded skills/program/SKILLS.md
skill_loader | loaded skills/employability/SKILLS.md
skill_loader | loaded skills/accommodation/SKILLS.md
skill_loader | loaded skills/news/SKILLS.md
skill_loader | loaded skills/forum/SKILLS.md
registry | hardcoded agent registered: scoring
registry | hardcoded agent registered: alternatives
registry | hardcoded agent registered: report_generator
registry | hardcoded agent registered: conversation
registry | research agent registered (skills scan): career
registry | research agent registered (skills scan): background
...
registry | build complete — 12 agents registered
```

**Drop-in agent discovery:**
Create `skills/test_agent/SKILLS.md` with `type: research`, restart → see
`registry | build complete — 13 agents registered`. Delete the folder, restart → back to 12.

This is the primary extensibility mechanism. New research domains, specialist sub-agents,
or experimental agents are added without touching any existing Python.

---

### 4.3 Responsibility Boundaries

Four classes own the system's top-level concerns. They do not overlap.
...
| `AgentRegistry` | `core/registry.py` | Constructed once at startup. Scans `skills/` directory, builds `SpecialistAgent` instances for research agents, registers hardcoded agents. Provides agent lookup by key and description listing for routing. |

---

### 4.4 Full Pipeline Flow

```
[ResearchRequestedMessage]
        ↓
[CareerResearchAgent]         — loaded from skills/career/SKILLS.md
  writes: board.career
  fires:  CareerResearchCompletedMessage
        ↓
[BackgroundAgent]   ──┐       — all loaded from skills/<name>/SKILLS.md
...
[ForumAgent]        ──┘
        ↓
[ScoringAgent]          quorum gate — waits for 7 section results (pass or fail)
  writes: board.score
...
### 4.5 Conversational Overlay

After the pipeline completes, the blackboard persists in the Chainlit session. A lightweight
`ConversationAgent` handles natural language follow-up questions answered from the research
data — not new searches.
...
`ConversationAgent` receives: user question + serialised blackboard as context + conversation
history via pydantic-ai `message_history`. It answers from what was already found.
No search tools. No new LLM research calls.
...
**Loaded from**: `skills/career/SKILLS.md`  
...
### 5.2 BackgroundAgent
**Phase**: 2 — parallel  
**Loaded from**: `skills/background/SKILLS.md`  
...
### 5.3 RankingsAgent
**Phase**: 2 — parallel  
**Loaded from**: `skills/rankings/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
...
### 5.4 ProgramAgent
**Phase**: 2 — parallel  
**Loaded from**: `skills/program/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
...
**Phase**: 2 — parallel  
**Loaded from**: `skills/employability/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
...
**Phase**: 2 — parallel  
**Loaded from**: `skills/accommodation/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
...
### 5.7 NewsAgent
**Phase**: 2 — parallel  
**Loaded from**: `skills/news/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
**Writes**: `board.news`  
...
### 5.8 ForumAgent
**Phase**: 2 — parallel  
**Loaded from**: `skills/forum/SKILLS.md`  
**Subscribes to**: `CareerResearchCompletedMessage`  
**Writes**: `board.forum`  
...
### 5.9 ScoringAgent
**Phase**: 3 — quorum gate  
**Hardcoded** (Python class) + `skills/scoring/SKILLS.md` for behavioural instructions  
...
**Hardcoded** (Python class) + `skills/alternatives/SKILLS.md` for behavioural instructions  
...
### 5.11 ReportGenerator
**Phase**: 3 — deterministic post-step, no LLM  
**Hardcoded** (Python class)  
**Subscribes to**: `AlternativesCompletedMessage`  
**Reads**: Full blackboard  
**Fires**: `ReportReadyMessage`  
**Produces**: `report.md`, `summary.md`, `sources.md`, `score.json` via Jinja2 templates

---

### 5.12 ConversationAgent
**Phase**: Post-pipeline (Chainlit session)  
**Hardcoded** (Python class) + `skills/conversation/SKILLS.md` for behavioural instructions  
**Receives**: User question + serialised blackboard + `message_history`  
**No tools** — answers from context only. No new searches.
...
The `ResearchHandler` injects into every agent's task brief:
...
```
university_research/
├── skills/                         SKILLS.md files — one per agent
│   ├── career/SKILLS.md
│   ├── background/SKILLS.md
│   ├── rankings/SKILLS.md
│   ├── program/SKILLS.md
│   ├── employability/SKILLS.md
│   ├── accommodation/SKILLS.md
│   ├── news/SKILLS.md
│   ├── forum/SKILLS.md
│   ├── scoring/SKILLS.md           instructions only (class is hardcoded)
│   ├── alternatives/SKILLS.md      instructions only (class is hardcoded)
│   └── conversation/SKILLS.md      instructions only (class is hardcoded)
│
├── core/
...
│   ├── registry.py                 AgentRegistry + build_registry()
│   └── skill_loader.py             SkillMeta parser + scan_skills_dir()
│
├── schemas/
...
│   ├── specialist.py               generic SpecialistAgent — built from SkillMeta, no subclasses
│   ├── scoring_agent.py            hardcoded — quorum gate logic
│   ├── alternatives_agent.py       hardcoded — reads board.score.weaknesses
│   └── conversation_agent.py       hardcoded — reads serialised blackboard, no tools
│
├── tools/
...
│
├── services/
│   └── research_handler.py         init deps, load registry, subscribe all agents, fire trigger
│
├── ui/
...
core/
  ...
  skill_loader.py     SkillMeta dataclass + load_skill() + scan_skills_dir()
  registry.py         AgentRegistry + build_registry() — stub mcp_server for now

skills/               create all SKILLS.md files (content stubs acceptable at this stage)
  career/SKILLS.md
  background/SKILLS.md
  rankings/SKILLS.md
  program/SKILLS.md
...
agents/
  ...
  specialist.py       SpecialistAgent — constructor takes (key, name, description,
                      instructions, mcp_server); no subclasses
```
...
# confirm all 8 research SKILLS.md files load without error
# confirm each returns a SkillMeta with key, name, type, description, instructions
# confirm a SKILLS.md with missing required field returns None and logs a warning
```
...
`SkillLoader` verified, `AgentRegistry` building cleanly from stubs, and
`BaseAgent` + `SpecialistAgent` pattern established. No LLM calls yet. No search tools yet.
...
```
skills/career/SKILLS.md     flesh out full instructions now — this is the template
                             agent that validates the SKILLS.md format for all others

agents/
  career_agent.py           first full agent, built as SpecialistAgent from
                            skills/career/SKILLS.md via build_registry()
                            subscribes to ResearchRequestedMessage
                            calls search_tool with university + course + country
                            produces CareerOutput
                            writes board.career
                            fires CareerResearchCompletedMessage
...
services/
  research_handler.py       subscribes career_agent only for now
                            loads registry via build_registry()
...
Skill loader round-trip:
  confirm: career agent instructions match skills/career/SKILLS.md body exactly
  confirm: career agent description matches SKILLS.md frontmatter description
...
One agent running end-to-end against real search data, loaded entirely from its
`SKILLS.md`. The full `build_registry() → ResearchHandler → MessageHub → Agent →
Blackboard → Message` cycle verified with real output. This is the pattern every
subsequent agent will follow — get it clean here.
...
Flesh out remaining SKILLS.md files and add section agents one at a time:

```
skills/background/SKILLS.md    + agents/ (built by registry from skill file)
skills/rankings/SKILLS.md      + agents/
skills/program/SKILLS.md       + agents/
skills/employability/SKILLS.md + agents/
skills/accommodation/SKILLS.md + agents/
skills/news/SKILLS.md          + agents/ (introduces sentiment classification)
skills/forum/SKILLS.md         + agents/ (hardest — multi-source, noise filtering)
```

Each section agent is a `SpecialistAgent` built by the registry from its SKILLS.md.
No Python class needed per agent — only the SKILLS.md and the registry wiring.

Add each to `ResearchHandler` subscriptions as its SKILLS.md is finalised.
Verify each agent individually before adding the next.

Then build Phase 3 (hardcoded classes):

```
agents/
  scoring_agent.py        quorum gate — counter logic, partial board handling
  alternatives_agent.py   reads board.score.weaknesses

skills/
  scoring/SKILLS.md       behavioural instructions for scoring criteria
  alternatives/SKILLS.md  behavioural instructions for gap-targeted selection
...
Skill loader verify:
  confirm startup log shows all SKILLS.md files loaded
  confirm registry build complete log shows correct agent count
  drop a skills/test_agent/SKILLS.md — confirm agent count increments at next start
  delete it — confirm agent count returns to baseline
...
All section agents loaded from SKILLS.md with no hardcoded agent subclasses.
Failure resilience confirmed. Drop-in agent discovery verified.
...
from research data — not new searches.
...
agents/
  conversation_agent.py   hardcoded class
                          reads skills/conversation/SKILLS.md for instructions
                          reads serialised blackboard as system context
                          maintains message_history across turns
                          no tools — answers from context only

skills/
  conversation/SKILLS.md  behavioural instructions — grounding rules,
                           how to handle questions outside the research scope
```
...
| SKILLS.md missing required field at startup | Warning logged, agent skipped, system continues |
| SKILLS.md body empty (no instructions) | Agent registered with base prompt only, no failure |
...
- Does `ConversationAgent` stay grounded in blackboard data or drift toward
  general knowledge? Prompt tuning via `skills/conversation/SKILLS.md` if needed —
...
- Invalid or missing SKILLS.md at startup — warning logged, agent skipped, system proceeds
...
`SPECIFICATIONS_3a.md` (Customer Service MAS Stage 3a) is the source of the
SKILLS.md-driven agent registry pattern. `SkillLoader`, `SkillMeta`, `AgentRegistry`,
and the `SpecialistAgent` generic constructor are adapted directly from that system.
The Telegram gateway from that reference is not used here — Chainlit is the UI layer.

Three patterns introduced here that do not exist in the v8 reference system:

1. **Quorum gate** on `ScoringAgent` — counter-based aggregator, proceeds
   at N regardless of pass/fail ratio (Section 4.2, Pattern 3)
2. **SKILLS.md-driven agent registry** — no research agent subclasses; agents are
   constructed dynamically from skill files at startup (Section 4.2, Pattern 4)
3. **Conversational overlay** on a batch pipeline — `ConversationAgent`
   answering from a persisted blackboard across multi-turn Chainlit history
   (Section 4.5)

Everything else is a translation of the reference systems into a new domain.
